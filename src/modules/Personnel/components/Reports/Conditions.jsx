    ...
    <div className="w-full ">
      {open && (
        <div className="flex flex-col gap-2 bg-white dark:bg-gray-800 p-4 rounded-md shadow-md">
          {conditions.map((cond, index) => {
            ...
            return (
              <div key={index} className="flex gap-2 items-center justify-around">
                <select className="w-full border border-gray-300 rounded px-2 py-1 text-sm" value={cond.field} onChange={(e) => handleChange(index, "field", e.target.value)}>
                ...
                {selectedCol?.type === "select" ? (
                  <select className="w-full border border-gray-300 rounded px-2 py-1 text-sm" value={cond.value} onChange={(e) => handleChange(index, "value", e.target.value)}>
                  ...
                  <input type="date" className="w-full border border-gray-300 rounded px-2 py-1 text-sm" value={cond.value} onChange={(e) => handleChange(index, "value", e.target.value)}  />
                ) : (
                  <input type="text" className="w-full border border-gray-300 rounded px-2 py-1 text-sm" value={cond.value} onChange={(e) => handleChange(index, "value", e.target.value)} placeholder="Değer girin" />
                )}
          ...
          <button onClick={handleAdd} className="flex font-bold text-sm items-center gap-1 text-blue-600 mt-1 cursor-pointer group transition-colors duration-300 ">
